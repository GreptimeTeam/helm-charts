...
![Version: 0.8.28](https://img.shields.io/badge/Version-0.8.28-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 1.1.4](https://img.shields.io/badge/AppVersion-1.1.4-informational?style=flat-square)
...
| auditLog | object | `{"classes":"ddl,admin","commands":"all","dir":"/data/greptimedb/audit-logs","enabled":false,"maxLogFiles":30,"objectTypes":"all","sources":"all"}` | Audit logging configuration This configuration use for GreptimeDB Enterprise, please do not enable it in OSS versions. Audit logs record operations executed on the database, helping monitor user activity, detect suspicious operations, and ensure compliance. see https://docs.greptime.com/enterprise/deployments-administration/monitoring/audit-logging/ |
| auditLog.classes | string | `"ddl,admin"` | Allowed statement classes for auditing. This option acts as a filter: if a statement's class does not match one of these configured values, it will not be recorded. Multiple classes are separated by commas (","). Available classes: "read", "write", "admin", "ddl", "misc". A special value "all" means all classes. Default: "ddl,admin". |
| auditLog.commands | string | `"all"` | Allowed statement commands for auditing. This option acts as a filter: if a statement's command does not match one of these configured values, it will not be recorded. Multiple commands are separated by commas (","). Available commands: "promql", "select", "copy", "insert", "delete", "create", "alter", "truncate", "drop", "admin", "misc". A special value "all" (default) means all commands. |
| auditLog.dir | string | `"/data/greptimedb/audit-logs"` | Directory to store audit log files. A new audit log file is generated daily. Default: "/data/greptimedb/audit-logs". |
| auditLog.enabled | bool | `false` | Enable the audit log plugin. Default: false. |
| auditLog.maxLogFiles | int | `30` | Maximum number of audit log files to retain. Default: 30. |
| auditLog.objectTypes | string | `"all"` | Allowed object types for auditing. This option acts as a filter: if the target object of a statement does not match one of these configured values, it will not be recorded. Multiple object types are separated by commas (","). Available object types: "database", "table", "view", "flow", "index", "misc", "trigger". A special value "all" (default) means all object types. |
| auditLog.sources | string | `"all"` | Allowed sources for auditing. This option acts as a filter: if a statement does not come from one of these configured sources, it will not be recorded in the audit log. Multiple sources are separated by commas (","). Available sources: "unknown", "http", "grpc", "mysql", "postgres". A special value "all" (default) means all sources. |
...
| base.podTemplate.main.envFrom | list | `[]` | Environment variables from secrets or configmaps to add to the pod |
...
| configMergeStrategy | string | `"ConfigMergeStrategyInjectedDataFirst"` | ConfigMergeStrategy is the strategy for merging the input config with the config that generated by the operator. Valid values: ConfigMergeStrategyOperatorFirst or ConfigMergeStrategyInjectedDataFirst. ConfigMergeStrategyOperatorFirst means the config that generated by the operator has higher priority than the input config. If the input config has the same key as the config that generated by the operator, the value of the input config will be ignored. ConfigMergeStrategyInjectedDataFirst means the input config has higher priority than the config that generated by the operator. If the input config has the same key as the config that generated by the operator, the value of the input config will be used. It should be used carefully because it may lead to some conflicts with the other components. It's the default strategy. |
...
| datanode.podTemplate.main.envFrom | list | `[]` | Environment variables from secrets or configmaps to add to the pod |
...
| flownode.podTemplate.main.envFrom | list | `[]` | Environment variables from secrets or configmaps to add to the pod |
...
| frontend.podTemplate.main.envFrom | list | `[]` | Environment variables from secrets or configmaps to add to the pod |
...
| image.tag | string | `"v1.1.4"` | The image tag |
...
| meta.podTemplate.main.envFrom | list | `[]` | Environment variables from secrets or configmaps to add to the pod |
...
| monitoring | object | `{"enabled":false,"logsCollection":{"pipeline":{"data":""}},"standalone":{},"ttl":"30d","vector":{"registry":"docker.io","repository":"timberio/vector","tag":"0.49.0-debian"}}` | The monitoring bootstrap configuration |
...
| monitoring.ttl | string | `"30d"` | The default retention time for monitoring data |
| monitoring.vector | object | `{"registry":"docker.io","repository":"timberio/vector","tag":"0.49.0-debian"}` | Configure vector for logs and metrics collection. |
...
| monitoring.vector.tag | string | `"0.49.0-debian"` | vector image tag |
